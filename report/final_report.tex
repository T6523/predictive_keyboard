...
\textbf{Cleaning hypothesis, tested false.} We have assumpation going in that stripping
...
accuracy gain, just more memory usage and training time. Separately, on the discarded
...
table, which does not scale to gigaword-sized corpora due to memory constraint. Replaced with
...
better-calibrated probabilities at no extra cost: +0.22pp word / +0.19pp
overall.

\textbf{Dedicated number KN5.} Numbers have few in-domain training examples,
so the general n-gram is comparatively weak there. So we built a second KN5 trained
...
decoding, letter-masked); Qwen2.5-3B was chosen as base model because it is much
smaller than Mistral-7B-v0.1 yet perform slightly ahead.
...
reaching only \textbf{39.83\% alphanumeric} --- worse than every zero-shot model in
...
+2.9pp word, and beating a single noisy 80/20 split's earlier 72.50\%
...
alone carries $\pm$2pp at 95\% CI, which is why the wider CV was run at all).
...
run at full scale. Due to compute constraint we need to split the inference between Kaggle and local session
\begin{itemize}
